...
const float cellScaleX = 10;
const float cellScaleY = 15;
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubvis = nh.advertise<visualization_msgs::Marker> ("/visualization_marker", 0);
  visualization_msgs::Marker rowDetection;
  rowDetection.header.frame_id = "camera";
  rowDetection.ns = "row_detection";
  rowDetection.type = visualization_msgs::Marker::ARROW;
  rowDetection.action = visualization_msgs::Marker::ADD;
  rowDetection.scale.x = 0.5;
  rowDetection.scale.y = 3;
  rowDetection.scale.z = 8;
  rowDetection.color.a = 1.0;
  rowDetection.color.r = 1.0;
  rowDetection.color.g = 0.0;
  rowDetection.color.b = 0.0;
  ros::Publisher pubpc = nh.advertise<sensor_msgs::PointCloud2> ("/row_detection_PC", 1);
  pcl::PointCloud<pcl::PointXYZ> rowDetectionPC(2, 1);
  rowDetectionPC.header.frame_id = "/camera";
  rowDetectionPC.points[0].y = 0;
  rowDetectionPC.points[1].y = 0;
  ...
  while (status) {
    ...
    if (newLaserPoints) {
      ...
      for (int i = 0; i < totalPointNum; i++) {
        if (pointCloud.points[i].y > -0.6 && pointCloud.points[i].y < 0.6) {
          dis = (pointCloud.points[i].x - intB * pointCloud.points[i].z) / den;
          if (dis - intA1den > -0.5 && dis - intA1den < 1.5) {
            cellIndX = cellScaleX * (dis - intA1den) + cellCenterXL;
            cellIndY = cellScaleY * pointCloud.points[i].y + cellCenterY;
            ...
            cellIndX = cellScaleX * (dis - intA2den) + cellCenterXR;
            cellIndY = cellScaleY * pointCloud.points[i].y + cellCenterY;
            if (cellIndX >= 0 && cellIndX < cellNumX && cellIndY >= 0 && cellIndY < cellNumY) {
      ...
      for (int i = 1; i < cellNumY - 1; i++) {
        offsetMeanXL[i] = (offsetMeanXL[i] - cellCenterXL) / cellScaleX * den;
        offsetMeanXR[i] = (offsetMeanXR[i] - cellCenterXR) / cellScaleX * den;
      }
      ...
      rowDetection.header.stamp = pointCloud.header.stamp;
      rowDetection.id = 1;
      rowDetection.pose.position.x = intA1R;
      rowDetection.pose.position.y = (minIDXL - cellCenterY) / double(cellScaleY); //-0.1;
      rowDetection.pose.position.z = 0;
      rowDetection.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0, atan(intB) - PI / 2, 0);
      pubvis.publish(rowDetection);
      rowDetection.id = 2;
      rowDetection.pose.position.x = intA2R;
      rowDetection.pose.position.y = (minIDXR - cellCenterY) / double(cellScaleY); //-0.1;
      pubvis.publish(rowDetection);

      rowDetectionPC.header.stamp = pointCloud.header.stamp;
      rowDetectionPC.points[0].x = intA1R * cos(atan(intB));
      rowDetectionPC.points[0].z = -intA1R * sin(atan(intB));
      rowDetectionPC.points[1].x = intA2R * cos(atan(intB));
      rowDetectionPC.points[1].z = -intA2R * sin(atan(intB));
      sensor_msgs::PointCloud2::Ptr rowDetectionPC2 (new sensor_msgs::PointCloud2());
      pcl::toROSMsg(rowDetectionPC, *rowDetectionPC2);
      pubpc.publish(*rowDetectionPC2);

      ROS_INFO ("Total: %d DS: %d Ori: %d Inliers: %d IDL: %d IDR: %d DIS: %f", 
                totalPointNum, totalPointNumDS, oriPointNum, inlierNum, minIDXL, minIDXR, intA1R - intA2R);
...
/*      //for (int i = 0; i < totalPointNum; i++) {
      ...
      //if (minIDXL >= 1 && minIDXL < cellNumY - 1) {
      //  cvCircle(image, cvPoint(240 - 25 * (offsetMeanXL[minIDXL] + intA1), 400 - 25 / double(cellScaleY) * (minIDXL - cellCenterY)), 
      //           1, CV_RGB(255, 0, 0), 4);
      //}
      //if (minIDXR >= 1 && minIDXR < cellNumY - 1) {
      //  cvCircle(image, cvPoint(240 - 25 * (offsetMeanXR[minIDXR] + intA2), 400 - 25 / double(cellScaleY) * (minIDXR - cellCenterY)), 
      //           1, CV_RGB(255, 0, 0), 4);
      //}
