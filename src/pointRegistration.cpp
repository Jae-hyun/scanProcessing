...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubpcds = nh.advertise<sensor_msgs::PointCloud2> ("/laser_points_Registered_DS", 1);
  ...
  while (status) {
    ...
    if (newLaserPoints) {
      ...
      sensor_msgs::PointCloud2::Ptr laserPointsReg (new sensor_msgs::PointCloud2());
      pcl::toROSMsg(pointCloudReg, *laserPointsReg);
      sensor_msgs::PointCloud2::Ptr laserPointsRegDS (new sensor_msgs::PointCloud2());
      pcl::VoxelGrid<sensor_msgs::PointCloud2> dsFilter;
      dsFilter.setInputCloud(laserPointsReg);
      dsFilter.setLeafSize(0.05, 0.05, 0.1);
      dsFilter.filter(*laserPointsRegDS);
      pubpc.publish(*laserPointsReg);
      pubpcds.publish(*laserPointsRegDS);
